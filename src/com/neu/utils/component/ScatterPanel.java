...
public class ScatterPanel extends JPanel implements Observer ,MouseListener,MouseMotionListener{
	
	public static void main(String [] args){
		
		JFrame frame = new JFrame();
		
		ScatterPanel scatterPanel = new ScatterPanel();
	...
	public ScatterPanel(Subject data) {
		this.data = data;
	...
	public void paint(Graphics g) {
		...
		if (flag == false) {
			...
			int x = NonInheritanceRefactoring.XCurrent;
			double y = NonInheritanceRefactoring.YCurrent;
...
//			System.out.println(dot.getColor().toString());
	...
	private void calculateColumn(int length){
		...
		int column = NonInheritanceRefactoring.Xtotal;
//		int column = 130;
	...
	private void calculateRow(int length) {
		this.maxValue = NonInheritanceRefactoring.Ymax;
		this.minValue = NonInheritanceRefactoring.Ymin;
//		this.maxValue = 0.45;
//		this.minValue = 0.23;
	...
	public void update(Suggestions sg) {		
		...
		if (isExist == false) {
			DotWithColor dot = calculateDot(x, y);
			dot.setUndoMethod(sg.getUndoEntityLength());
			dot.setUndoClass(sg.getUndoClassLength());
			dotList.add(dot);
		}
	...
	private static JPopupMenu getPopup(final JPanel panel, boolean isDotScope, final DotWithColor dot) {
		...
		if (isDotScope == true) {
			...
			delete.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					((ScatterPanel) panel).getDotList().remove(dot);
					panel.repaint();
				}
			});
			popup.add(delete);
		...
		clear.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				((ScatterPanel) panel).getDotList().clear();
				
				Suggestions sg = NonInheritanceRefactoring.sgs;
				sg.setUndoClassLength(0);
				sg.setUndoEntityLength(0);
				((Data)data).setMeasurements(sg);
		...
		if (e.getClickCount() == 1) {
			...
			for (int i = 0; i < dotList.size(); i++) {
				...
				if (mouseX>dot.x && mouseX<dot.x+dotWidth && mouseY>dot.y && mouseY<dot.y+dotHeight) {
					...
					Suggestions sg = NonInheritanceRefactoring.sgs;
					sg.setUndoClassLength(dot.getUndoClass());
					sg.setUndoEntityLength(dot.getUndoMethod());
					((Data)this.data).setMeasurements(sg);
					return;
				}
			}
		}
